...
df_other_query = isbn.query("categories.str.contains('Engineering') or description.str.contains('engineering') or description.str.contains('technology') and `price` >= 0.0", engine="python")
...
df_mental_help = isbn.query("categories.str.contains('Mystery') and (description.str.contains('murder') or description.str.contains('killer') or description.str.contains('assassination'))", engine="python")
df_mental_help = df_mental_help.sort_values(by=["page_count"], ascending=[True])
df_query8 = isbn.query("authors.str.contains('Tim ', na=False) or authors.str.contains('Tom ', na=False)", engine="python")
df_query7 = isbn.query("description.str.contains(' son', na=False) or description.str.contains(' sun', na=False) or description.str.contains(' sin', na=False) or title.str.contains('man', na=False)", engine="python")
df_query6 = isbn.query("description.str.contains('must read', na=False) or description.str.contains('essential read', na=False) or description.str.contains('essential information', na=False) or description.str.contains('important book', na=False) or description.str.contains('essential book', na=False) or description.str.contains('must have', na=False) or description.str.contains('must-read', na=False)")
...
f = open('evaluation/q8rels.txt', 'w')

for x in df_query8['title']:
    f.write(x + "\n")

f.close()

f = open('evaluation/q7rels.txt', 'w')

for x in df_query7['title']:
    f.write(x + "\n")

f.close()

f = open('evaluation/q6rels.txt', 'w')

for x in df_query6['title']:
    f.write(x + "\n")
